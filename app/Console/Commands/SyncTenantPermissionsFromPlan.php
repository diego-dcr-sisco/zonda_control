...
class SyncTenantPermissionsFromPlan extends Command
{
    ...
    protected function syncTenant($tenantId)
    {
        ...
        $this->info("Sincronizando tenant: {$tenant->company_name} (ID: {$tenant->id})");
     ...
     */
    protected function syncPermissionsForTenant(Tenant $tenant)
    {
        ...
        $allPermissions = \Spatie\Permission\Models\Permission::pluck('id')->toArray();

        // Eliminar registros antiguos
        TenantPermissionControl::where('tenant_id', $tenant->id)->delete();
